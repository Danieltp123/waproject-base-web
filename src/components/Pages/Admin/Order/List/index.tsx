...
export default class UserListPage extends ListComponent<{}, IState> {
  ...
  handleEdit = (current: IOrder) => {
    this.setState({ formOpened: true, current });
  };

  formCallback = (order?: IOrder) => {
    this.setState({ formOpened: false });

    this.state.current ? this.loadData() : this.handleChangeTerm(order.description);
  };
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCellSortable {...this.sortableProps} column='quantity'>
                    Quantidade
                  </TableCellSortable>
              ...
              <TableBody>
                ...
                {items.map(order => (
                  <ListItem key={order.id} order={order} onEdit={this.handleEdit} onDeleteComplete={this.loadData} />
                ))}
              </TableBody>
            </Table>
          </TableWrapper>
